 ...
 * expand to fill available space except when the horizontal alignment is <code>BOTH</code> in
 * which case components are stretched horizontally. Unlike FlowLayout, components will not wrap
 * to form another column if there isn't enough space vertically. VerticalLayout can optionally
 * anchor components to the top or bottom of the display area or center them between the top and
 * bottom.
 *
 * Revision date 12th July 2001
 *
 * @author Colin Mummery e-mail: colin_mummery@yahoo.com Homepage:www.kagi.com/equitysoft - Based
 * on 'FlexLayout' in Java class libraries Vol 2 Chan/Lee Addison-Wesley 1998
 */
...
// Constructors
    ...
    /**
     * Constructs a VerticalLayout instance with horizontal centering, anchored to the top with
     * the specified vgap
     *
     ...
     *
     * @param vgap An int value indicating the vertical seperation of the components
     * @param alignment An int value which is one of <code>RIGHT, LEFT, CENTER, BOTH</code> for
     ...
     * @param alignment An int value which is one of <code>RIGHT, LEFT, CENTER, BOTH</code> for
     * the horizontal alignment.
     * @param anchor An int value which is one of <code>TOP, BOTTOM, CENTER</code> indicating
...
// -----------------------------------------------------------------------------
    ...
    {
        return getClass().getName() + "[vgap=" + vgap + " align=" + alignment + " anchor="
            + anchor + "]";
    }
}